...
def re_ranking(probFea, galFea, k1, k2, lambda_value, local_distmat=None, only_local=False, pbar = None) -> torch.Tensor:
    ...
    progress_bar = range(all_num)

    if isinstance(pbar, gr.Progress):
        progress_bar = pbar.tqdm(progress_bar)
    else:
